...
__all__ = ['ResTensorVisionTransformer']  # model_registry will add each entrypoint fn to this
...
class TensorizedLinear(nn.Module):
    def __init__(self, indim=768, outdim=768, middim=[12,8,8,8,8,12], 
                 ...
                 activation='GELU', InterRes=False, ResGate=False, 
                 TenserizedGate=False, LatentAct=True, bn=False, fact=False, bias=True):
        ...
        #-----Cores-----#
        self.cores = []
        for i_core in range(len(self.middim)):
        ...
        if ResGate is True:
            ...
            self.resgate = nn.ParameterList()
            self.resgateShape = None
            if TenserizedGate:
                self.TensorizeGate()
            else:
                self.resgate = nn.Linear(self.ranks[len(self.ranks)//2],self.ranks[len(self.ranks)//2])
        self.initparam()
        #self.bn = nn.BatchNorm1d(self.outdim)
        if bias is True:
    ...
    def TensorizeGate(self):
        self.resgateShape = most_balanced_factor_pair(self.ranks[len(self.ranks)//2])
        resgate_encoder = nn.Parameter(nn.init.kaiming_uniform_(torch.Tensor(max(self.resgateShape),max(self.resgateShape))))
        resgate_decoder = nn.Parameter(nn.init.kaiming_uniform_(torch.Tensor(min(self.resgateShape),min(self.resgateShape))))
        self.resgate.append(resgate_encoder)
        self.resgate.append(resgate_decoder)

    def initparam(self):
        for core in self.cores:
            nn.init.kaiming_normal_(core,nonlinearity='relu')

    def LinearContractionForwardInner(self,x,res_former):
        print('using inner')
        #todo
        return x, None
    
    def ResGateForward(self,res):
        if self.resgateShape is not None:
            x = contract('abcd,de,cf-> abfe', res.view(-1,1,self.resgateShape[0],self.resgateShape[1]), self.resgate[0],self.resgate[1])
        else:
            x = self.resgate(res)
        return x
    ...
    def forward(self,x,res_former=None):
        #x = x.reshape(-1,*self.InputShape)
        x, ResTensorCores = self.InterRes_LinearContractionForward(x,res_former)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.to_q = TensorizedLinear(dim, dim, middim, rank, InputShape, activation,
                                     InterRes_Pos['q'], ResGate_Pos['q'], TenserizedGate_Pos['q']
                                     ,LatentAct_Pos['q'], bn_Pos['q'], fact_Pos['q'], qkv_bias)
        self.to_k = TensorizedLinear(dim, dim, middim, rank, InputShape, activation,
                                     InterRes_Pos['k'], ResGate_Pos['k'], TenserizedGate_Pos['k']
                                     , LatentAct_Pos['k'],bn_Pos['k'], fact_Pos['k'],qkv_bias)
        self.to_v = TensorizedLinear(dim, dim, middim, rank, InputShape, activation,
                                     InterRes_Pos['v'], ResGate_Pos['v'], TenserizedGate_Pos['v']
                                     ,LatentAct_Pos['v'],bn_Pos['v'],fact_Pos['v'],qkv_bias)
        ...
        self.proj = TensorizedLinear(dim, dim, middim, rank, InputShape, activation,
                                              InterRes_Pos['out'], ResGate_Pos['out'],TenserizedGate_Pos['out']
                                              ,LatentAct_Pos['out'],bn_Pos['out'],fact_Pos['out'])
...
class Block(nn.Module):
    def __init__(
        ...
        InterRes_Pos, ResGate_Pos = blk_config.get('InterRes_Pos'), blk_config.get('ResGate_Pos')
        ...
        if first:
            ...
            ResGate_Pos = {key: False for key in ResGate_Pos}
        ...
        self.fc1 = TensorizedLinear(dim, mlp1_outdim, mlp1_middim, mlp1_rank, InputShape, activation,
                                     InterRes_Pos['mlp1'], ResGate_Pos['mlp1'], TenserizedGate_Pos['mlp1']
                                     ,LatentAct_Pos['mlp1'],bn_Pos['mlp1'],fact_Pos['mlp1'],mlp1_bias)
        if activation == 'ReLU':
            self.act = nn.ReLU()
        ...
        self.fc2 = TensorizedLinear(mlp1_outdim, dim, mlp2_middim, mlp2_rank, mlp2_InputShape, activation,
                                     InterRes_Pos['mlp2'], ResGate_Pos['mlp2'], TenserizedGate_Pos['mlp2']
                                     ,LatentAct_Pos['mlp2'],bn_Pos['mlp2'],fact_Pos['mlp2'],mlp2_bias)
...
class ResTensorVisionTransformer(nn.Module):
    """ Vision Transformer
...
@torch.no_grad()
def _load_weights(model: ResTensorVisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
...
def _convert_openai_clip(
        ...
        model: ResTensorVisionTransformer,
        prefix: str = 'visual.',
...
def _create_restensor_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
...
@register_model
def plaintensor_vit_base_patch16_224(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/ptt.yaml'
    ...
    model = _create_restensor_vision_transformer('plaintensor_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def restensor_vit_plus_patch16_224(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/restensor_vit_plus_patch16_224.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=768, depth=16, num_heads=12, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_plus_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def restensor_vit_base_patch16_224(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/rtt_trg_act_bn_nfact_12h_nbn.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def restensor_vit_base_patch16_224_ablation_NoneRes(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/base_16_224_noneres.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_base_patch16_224_ablation_NoneRes', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def restensor_vit_base_patch16_224_ablation_NoneAct(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/base_16_224_noneact.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_base_patch16_224_ablation_NoneAct', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def restensor_vit_base_patch16_224_ablation_Rank128(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """
    Important Note: model config is create only by blk_config, but other args should be consistant to blk_config.
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/base_16_224_R128.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_base_patch16_224_ablation_Rank128', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def restensor_vit_small_patch16_224(pretrained: bool = False, **kwargs) -> ResTensorVisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    blk_config = '/home/ruijiezhang/ResTensor/config/restensor_vit_small_patch16_224.yaml'
    layerconfig = _read_blk_config(blk_config)
    model_args = dict(patch_size=16, embed_dim=512, depth=12, num_heads=8, blk_config=layerconfig)
    model = _create_restensor_vision_transformer('restensor_vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

register_model_deprecations(__name__, {
    'restensor_vit_base_patch16_224': 'restensor_vit_base_patch16_224',
})
